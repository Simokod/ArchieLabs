...
; eax - al as Y, in inner loop as temp
  ...
  .Y_check:
    ...
    cmp al, 0xC8                    ; check if Y operand exceeds 200 (= 0xC8)
    ja failed
...
; eax - al as Y, in inner loop as prev link
    ...
    .cont_loop:
      ...
      .shift_loop:  
       ...
       .cont:
          ...
          adc byte [carry_flag_value], 0    ; updating value of carry_flag_value
          cmp byte [carry_flag_value], 1    ; checking if there was carry in previous calculation
          jne .no_carry

          cmp eax, 0                        ; checking if first link
          je .no_carry
          add byte [eax], 0x80              ; adding carry value to link
      ...
      free_last_link:
        mov ebx, dword [current_size]     ; for computation purposes
        mov edx, dword [stack+ebx*4-4]   ; moving first link address to edx
        cmp edx, ecx                      ; check if first link is curr link
        je skip_free                      ; skip free
  ...
  count_bits:
    ...
    finish_one_bits:
      ...
      mov eax, [new_link_address]       ; for computation purposes
      mov ebx, dword [current_size]     ; for computation purposes
      mov dword [stack+ebx*4-4], eax    ; putting list in stack
      mov eax, dword [one_bits_counter]  ; for computation purposes

      .loop:
        mov edx, 0
        mov ebx, 0x100
        div ebx                           ; getting 2 LSB

        mov ecx, dword [new_link_address] ; for computation purposes
        mov byte [ecx], dl                ; storing number in link
        ...
        add esp, 8                            ; remove pushed argument
        mov ecx, dword [new_link_address]     ; for computation purposes
        mov dword [ecx+1], eax                ; chaining links
        mov dword [new_link_address], eax     ; saving returned adress
        popad
...
after_rec:
      ...
      cmp ecx, 0xf          ; checking if only 1 digit
      jg print_normally
